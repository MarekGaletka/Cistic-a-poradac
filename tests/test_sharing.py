...
def test_password_hash_stored(cat):
    ...
    found = cat.get_share_by_token(share["token"])
    # Password is now stored as PBKDF2 hash with salt and iterations: "salt_hex:600000:dk_hex"
    ph = found["password_hash"]
    assert ph.count(":") == 2, "PBKDF2 hash should be salt:iterations:dk"
    salt_hex, iter_str, dk_hex = ph.split(":", 2)
    assert iter_str == "600000"
    # Verify the hash by re-deriving with the stored salt
    salt = bytes.fromhex(salt_hex)
    expected_dk = hashlib.pbkdf2_hmac("sha256", b"test123", salt, 600_000)
